...
public class GrainTypeConvertor 
{
    ...
    public boolean isTypeSupported(String type)
    {
        ...
        for (GrainTypeSet gts : types )
        {
            for (String s : gts.getDescriptions())
            {
               if (type.equals(s)) return true;
            }
        }
    ...
    {
        uitype = uitype.trim();
        if (uitype.equals("Faceted Crystals")) return basicTypeCodes[FC]; 
        if (uitype.equals("Decomposing & fragmented precip. particles")) return basicTypeCodes[DF];
